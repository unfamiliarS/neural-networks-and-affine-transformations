...
# Веса и смещения из вашего вывода
...
# Создаем одинаковый диапазон для X и Y
x_min, x_max = df['x'].min() - 0.5, df['x'].max() + 0.5
y_min, y_max = df['y'].min() - 0.5, df['y'].max() + 0.5

# Используем одинаковое количество точек для равномерного отображения
x_range = np.linspace(x_min, x_max, 100)

# Линии для 2 нейронов скрытого слоя
for neuron_idx in range(2):
    ...
    if abs(w2) > 1e-6:
        ...
        # Фильтруем точки, которые попадают в диапазон Y
...
# Линия выходного слоя (граница принятия решений)
